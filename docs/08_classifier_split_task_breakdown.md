...
| **T1.4** | `lib/mix/tasks/credence.equiv.ex` (new) | **`mix credence.equiv` — the behavioural-equivalence check (`07` §3.11).** Reads a `before` snippet + a rule module; compiles `before`, computes `fix(before)`, **compiles the fix** (non-compiling ⇒ `DIVERGES`), then calls each public function with a **fixed adversarial battery** matched to arity (empty/single/short list, `Range`, **>32-key** & **present-key** maps, `nil`, `-1`, `0`, `7`, `"7"`, `"10"`, ASCII / combining-accent / multi-codepoint-emoji / flag strings, a side-effecting closure) and requires **same value OR same exception class** on every input. Prints `EQUIVALENT` / `DIVERGES <input> <before> <after>`. **Fixed battery, not StreamData** (determinism). **Pattern-phase only** (needs a compilable `before`). Names no rule; accepts the same `defmodule` snippet form as `covers`. **`--assumptions` (`07` §3.12):** runs the rule in that Credence mode and **filters the battery to the admitted domain** (a `single_codepoint_graphemes` run drops the combining-accent/emoji/flag inputs, keeps the rest); run under `:strict` + each registered switch it reports the **minimal switch set** that makes `before ≡ fix` (∅ = no-promise, a set = switch-gated, "none works" = bug-or-new-switch). Reads `Credence.Assumptions` (never writes it). | Dogfood: a known-unsafe `followup.md` snippet (`no_if_subtraction_for_max` on a non-number, `no_enum_slice_with_length` on `[]`) ⇒ `DIVERGES` under *all* modes; a rare-text one (`no_codepoint_string_reverse`) ⇒ minimal set `{single_codepoint_graphemes}`; a true no-op ⇒ minimal set ∅. |
...
| **T4.3** | `lib/tunex/equiv.ex` (new) + `lib/tunex/evolve/gate.ex` | **Behavioural-equivalence check (`07` §3.11/§3.12) — pattern-phase, TWO run-points.** (a) **Classify-time pre-check** (for a fixable `POTENTIAL_NEW_RULE`/BUGFIX, after novelty): shell `mix credence.equiv` (with the spec's `assumptions`, `07` §3.12) in clone on the classifier's `before`/`after`; it returns the **minimal switch set** — ∅ ⇒ proceed no-promise; a set ⇒ proceed switch-gated (overriding/correcting the spec tag); `DIVERGES` under all registered switches ⇒ **`NO_ACTION`** → `behaviour_diverged/` (or `SWITCH_PROPOSAL` if the classifier flagged a clean rare-text class, T4.4), skip implementer. (b) **6th Gate check**: after the implementer writes the rule, run `mix credence.equiv --assumptions <rule's>` on `before` vs the **actual `fix(before)`**; `DIVERGES` → Gate reject → `escalated/`. Non-pattern phases skip both. | A behaviour-diverging proposal never builds; the minimal-set decides no-promise vs switch-gated; an implementer that broadens onto an admitted-input divergence fails the 6th Gate check; a true (partial-)no-op passes. |
...
| **T5.2** | `lib/tunex/implement/output.ex` (new) | **File-keyed parser**: generic `===KEY===`→`%{key=>content}`. New = `RULE` + `CHECK_TEST` + `FIX_TEST` — **all three required** (every rule is fixable, `07` §4.1; a missing `FIX_TEST` is an invalid emit, not a check-only rule) **+ `PROPERTY_TEST` REQUIRED iff the spec has `assumptions` (`07` §3.12 Tier 1)** → written to `test/pattern/<rule>_property_test.exs` (the `<Rule>PropertyTest` the 0.7.0 meta-test demands). Bugfix = `RULE` + `TEST:<path>` (each ⊆ glob, modify-only; reject new/renamed). | New-mode (all required blocks) + bugfix-mode samples parse; a switch-gated emit missing `PROPERTY_TEST` is rejected; a no-promise emit with a stray `PROPERTY_TEST` is rejected; out-of-glob `TEST:` path rejected. |
