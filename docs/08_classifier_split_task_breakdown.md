...
> ## ★ BUILD STATUS (2026-06-08) — Phases 0–6 BUILT + green; 7–8 are live-gated
> Implemented end-to-end with unit + clone-integration tests (Tunex: 73 tests/4 doctests green; Credence:
> 4397 tests green). Inline **✅ BUILT** notes record where the implementation refined the plan.
> - **Phase 0** ✅ `:classify`/`:implement` stages+floors+guards, `:anthropic_opus` provider+price, per-stage
>   `Budget.record` tagging (+`mix tunex.usage` split), `classify_endpoint_reachable!` preflight smoke.
> - **Phase 1** ✅ [C] `credence.{ast,covers,equiv}` + T1.3b full-diagnostic + T1.3 revert log_diff —
>   **committed to the clone** (`14aea49`) so `reconcile!`/`discard` can't wipe it. `equiv` is the trichotomy,
>   expression-level, `MIX_ENV=test`, dogfooded.
> - **Phase 2** ✅ `===SOLVE_BOUNDARY===` sentinel, `Distill`, `AppliedRules`, `RulePaths`.
> - **Phase 3** ✅ `Classify.{Prompt,Parser,Spec}` + `Classify` (option-shaping, lens fork, verbatim §3.10
>   blocks, assumptions registry via `Tunex.Credence`, validation gates + one re-ask). Marker splitter `Markers`.
> - **Phase 4** ✅ `Novelty` (covers), `Equiv` (module→expression bridge + trichotomy/minset/skip),
>   `SwitchProposal` + `mix tunex.switch_proposals`.
> - **Phase 5** ✅ `Implement.{Naming,Output,Seed}` + `Implement` loop (scaffold read-back, REAL_DIAGNOSTIC,
>   ceilings, flat retry) + config knobs. Scaffold honest-red verified against the real clone.
> - **Phase 6** ✅ `Evolve.Router` (the spine — all lanes + outcome dirs + ledger write-sites), RowLog outcome
>   moves, `tunex.reset` dirs, orchestrator calls the Router with the solve-outcome lens.
> - **Phase 7–8** ⏳ **LIVE-GATED** (needs Mimo creds + GPU Qwen): prove 2–3 rules land, THEN delete
>   `claude_code.ex` + the agentic `credence_rule_generator` + `max_turns` (teardown is gated on the proof),
>   then measure vs 4–8×. The agentic path is already **bypassed** (Router replaced its call) but not yet deleted.
>   ⚠️ For a live prove-run before teardown: the classifier/implementer use Mimo-pro via `LLM.for_stage`; the
>   old `cc_smoke!` preflight check still runs (CC is now unused) — keep CC creds valid or comment that line out.
...
>
> **🔴 The Phase-1 Credence infra MUST be committed to the clone** (it is a real PR, after all): `Preflight.reconcile!`
> and `Gate.discard` both run `git reset --hard HEAD` + `git clean -fd` on the clone, which **deletes untracked
> files + reverts unstaged edits** — so uncommitted tasks (`credence.{ast,covers,equiv}.ex`) and log tweaks
> (T1.3/T1.3b) get wiped at the next boot or gate-reject. (Learned the hard way: a test's blanket
> `git checkout . && git clean -fd` cleanup once erased the whole Phase-1 PR mid-build.) Corollary: **never
> blanket-clean the clone while it carries uncommitted work** — cleanup must be surgical (delete only the
> specific generated files). The Gate's discard is fine *because* it runs only on the rule files it just wrote.
>
...
| **T4.3** | `lib/tunex/equiv.ex` (new) + `lib/tunex/evolve/gate.ex` | **✅ BUILT (2026-06-08): `Tunex.Equiv` bridges the module-form spec → expression-level `credence.equiv`.** Since the spec's `before`/`after` are `defmodule`s but `credence.equiv` is expression-level, `Equiv.extract/1` pulls the single public `def`'s **param names (→ `--vars`)** + **body expression** from a simple single-clause module (handles `when` guards); pattern-destructured params / multi-clause / >1 def / multi-var ⇒ `:error` ⇒ `check/2` returns **`:skipped`** (T2 — deferred to the Gate's per-rule `_equivalence_test`). Verdicts mapped: `{:equivalent, minimal_switch_set}` (the set **overrides** the spec's `assumptions` tag) / `{:repair, evidence}` / `{:diverges, detail}` / `:skipped`. Shells `MIX_ENV=test mix credence.equiv … --minimal-set` via `Tunex.Credence.equiv/4` (temp files; `System.cmd` has no stdin, so `covers`/`equiv` write temp `.exs`). Integration-tested against the real clone (DIVERGES/REPAIR/EQUIVALENT+minset/skipped). **Behavioural-equivalence check (`07` §3.11/§3.12) — pattern-phase, TWO run-points.** (a) **Classify-time pre-check** (for a fixable `POTENTIAL_NEW_RULE`/BUGFIX, after novelty): shell `MIX_ENV=test mix credence.equiv` (with the spec's `assumptions`, `07` §3.12) in clone on the classifier's `before`/`after`; it returns the **trichotomy verdict + minimal switch set**. **`EQUIVALENT`:** ∅ ⇒ proceed no-promise; a set ⇒ proceed switch-gated (overriding/correcting the spec tag). **`REPAIR`** (`before` raises on every admitted input, after succeeds — §3.10) ⇒ proceed to implementer in **repair sub-mode** (emits `mark_equivalence_repair` seeded with the verdict's exception-class + `N/N` evidence; **not** `behaviour_diverged/`). **`DIVERGES`** under all registered switches ⇒ **`NO_ACTION`** → `behaviour_diverged/` (or `SWITCH_PROPOSAL` if the classifier flagged a clean rare-text class, T4.4), skip implementer. (b) **6th Gate check — SUBSUMED (2026-06-08), no new Gate code.** The built rule's behaviour is checked by its **mandatory `_equivalence_test.exs`** (Credence's hard `equivalence_meta_test`; runs the **actual `fix(before)`** over the `EquivalenceInputs` battery, strict `===`), which the Gate's existing full `mix test` already runs — a diverging fix fails its own test → suite RED → reject → `escalated/`. So `gate.ex` needs **no `credence.equiv` invocation**; the work moves to T5.1/T5.2 (the implementer must *author* a real equivalence test, anti-stub-enforced). Non-pattern phases have no equivalence test (their §3b gates apply instead). | A behaviour-diverging proposal never builds (a); a `REPAIR` verdict (always-crashes `before`) proceeds in repair sub-mode and lands with `mark_equivalence_repair`, NOT `behaviour_diverged/`; the minimal-set decides no-promise vs switch-gated; an implementer that broadens onto an admitted-input divergence fails the rule's own equivalence test in the Gate suite; a true (partial-)no-op passes. |
